...
def animate_network(history: List[Dict], G: nx.Graph, interval_ms: int = 600, figsize=(6, 6), metric_label: str = "Score", metric_id: Optional[str] = None):
    """Return a matplotlib.animation.FuncAnimation showing node score changes over rounds.

    Node color encodes score [0,1] using viridis; edges drawn lightly. Pass the
    ...
    def init():
        ...
        colors = plt.cm.viridis(np.array(scores0))
    ...
    def update(frame):
        ...
        node_scatter.set_color(plt.cm.viridis(np.array(scores)))
...
def show_animation(history: List[Dict], G: nx.Graph, interval_ms: int = 600, figsize=(6, 6), metric_label: str = "Score", metric_id: Optional[str] = None):
    """Display the animation; uses JS HTML in notebooks, falls back to plt.show()."""
    ani = animate_network(history, G, interval_ms=interval_ms, figsize=figsize, metric_label=metric_label, metric_id=metric_id)
    try:
